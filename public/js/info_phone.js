...
const phone = document.querySelector('#phone-vl')

const INSERT_USER = 'http://localhost:3000/users_phone'
...
nextElement.addEventListener('click',(e) => {
    ...
    if(!formElement[2].value.trim().match(mailformat)){
        alertify.alert('Có lỗi xảy ra','Email không hợp lệ').set('resizable',true).resizeTo(250,250)
            return
    }
    ...
    if(formElement[4].value.trim() !== formElement[5].value.trim()){
        ...
        const data = {
            ...
            'email' : formElement[2].value.trim(),
